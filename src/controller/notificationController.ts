...
export class NotificationController {
  ...
  static getNotifications = async (req: Request, res: Response) => {
    ...
    const notifications = await notificationService.getNotifications(query, user);
  ...
  static readNotification = async (req: Request, res: Response) => {
    const { notificationId } = (req as ValidatedParamsRequest<NotificationIdParamsType>).validatedParams;
    const user = req.user as User;
    const notification = await notificationService.readNotification(notificationId, user);
