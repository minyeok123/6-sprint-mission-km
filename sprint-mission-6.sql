...
select * from orders where date = DATE'2025-03-12'

select * from orders where extract(month from date) = 3 AND extract(day from date) = 12;
...
select pizza_id , count(pizza_id) as all_quantity 
from order_details 
group by pizza_id;

-- 3. `pizzas` 테이블에서 `price`의 크기가 20보다 큰 피자의 종류만 `order_details` 테이블에서 조회하세요. (힌트: 서브쿼리)
select pizza_id 
from order_details where pizza_id IN (select id from pizzas where price>20) 
group by pizza_id;

-- 4. `orders` 테이블에서 각 날짜별 총 주문 건수를 `order_count` 라는 이름으로 계산하고, 
-- 하루 총 주문 건수가 80건 이상인 날짜만 조회한 뒤, 주문 건수가 많은 순서대로 정렬하세요.
 select date, count(date) as order_count from orders 
 group by date having count(date) >= 80 
 order by count(date) desc;

-- 5. `order_details` 테이블에서 피자별(`pizza_id`) 총 주문 수량이 10개 이상인 피자만 조회하고, 총 주문 수량 기준으로 내림차순 정렬하세요.
 select pizza_id, count(pizza_id) as order_count from order_details 
 group by pizza_id having count(pizza_id) >=10 
 order by count(pizza_id) desc ;

-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. (수익 = `quantity * price`)
select order_details.pizza_id,count(order_details.pizza_id),sum(order_details.quantity * pizzas.price) as toal_revenue
from order_details 
join pizzas on order_details.pizza_id = pizzas.id 
group by order_details.pizza_id , pizzas.id;
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
select pizzas.id,pizzas.type_id,pizzas.size,pizzas.price,
sum(order_details.quantity) as total_quantity
from pizzas 
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
    select orders.date, count(distinct orders.id) as total_orders 
...
/*
    ...
    수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
select pizza_types.name as pizza_name , pizzas.size as pizza_size
, pizzas.price as pizza_price, order_details.quantity , (order_details.quantity * pizzas.price) as total_amount
from order_details
join pizzas on pizzas.id = order_details.pizza_id
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
select pizzas.size , sum(order_details.quantity * pizzas.price) 
from order_details
join pizzas on order_details.pizza_id = pizzas.id as total_amount
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
select  pizza_types.name ,  sum(order_details.quantity * pizzas.price) as total_amount
from order_details
join pizzas on order_details.pizza_id = pizzas.id
